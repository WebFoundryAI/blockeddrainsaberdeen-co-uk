...
function generateSitemapUrls(): SitemapUrl[] {
  ...
  for (const [i, location] of INDEXED_LOCATIONS.entries()) {
    // Main location page (Aberdeen gets higher priority)
    const isAberdeen = location.slug === 'aberdeen';
    urls.push({
      loc: `/locations/${location.slug}/`,
      priority: isAberdeen ? 0.9 : 0.8,
      changefreq: 'weekly',
      lastmod: getStaggeredDate(isAberdeen ? 1 : 2 + i),
    });
